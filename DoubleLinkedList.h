...
protected:
	struct Node
	{
		...
		Node() : pNext(nullptr), pPrev(nullptr), Data(0) {};
...
public:
	...
	void Insert(size_t ID, DATA_TYPE Data)
	{
	...
	void SetItem(size_t ID, DATA_TYPE Data)
	{
		// Limitting the ID between (0) to (m_Size - 1)
		ID = max(ID, 0);
		ID = min(ID, m_Size - 1);
	...
	auto GetItem(size_t ID) const->DATA_TYPE
	{
		// Limitting the ID between (0) to (m_Size - 1)
		ID = max(ID, 0);
		ID = min(ID, m_Size - 1);
	...
	void WillPrintLog(bool Value)
	{
		m_bPrintLog = Value;
	}

	void WillPrintListAlways(bool Value)
	{
